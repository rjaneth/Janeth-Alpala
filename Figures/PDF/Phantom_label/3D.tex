\documentclass[tikz, border=0.0cm]{standalone}
\usepackage{tikz}

\begin{document}
\begin{tikzpicture}[inner sep=0pt]
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=0.7\textwidth]{../3d2}};
    \useasboundingbox (image.south west) rectangle (image.north east);
\end{tikzpicture}%
\end{document}
...
%%\node [anchor=east] (right) at (11.0, 6.2) {%
%%\begin{tabular}{@{}l@{}}
%%\vspace{-0.5em}
        %%\tiny{$L=1$} \\
    %%\vspace{-0.2em} \tiny{$L=3$} \\
    %%\vspace{-0.1em} \tiny{$L=8$}\\
		 %%\vspace{-0.1em} \tiny{$L=12$}\\
		%%\vspace{-0.2em} \tiny{$L=100$}
    %%\end{tabular}
%%};%{ $\alpha=-2$,\, $\mu=30$};
...
%\begin{scope}[xshift=1.5cm]
    %\node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=0.7\textwidth]{../sim_phantom}};
    %\begin{scope}[x={(image.south east)},y={(image.north west)}]        
        %\draw [-latex, line width=1pt, black] (note) -- ++(0.3,0.0);
