...
In this work, we do not focus on parameter estimation using likelihood-based or moment-based methods. Instead, we adopt a testing framework to detect heterogeneity in SAR imagery. The analytical expressions of entropy measures derived from the $\Gamma_{\text{SAR}}$ and $\mathcal{G}_I^0$ models form the theoretical basis for constructing these statistical tests. In the following section, we introduce the entropy-based measures, specifically Shannon, Rényi, and Tsallis, on which our detection strategy is built.
...
\textbf{Tsallis entropy},  was first introduced by Havrda and Charvat&nbsp;[-@Havrda1967]  in the context of information theory. Then, Tsallis&nbsp;[-@Tsallis1988] exploited its non-extensive features and placed it in a physical setting, known as generalized Tsallis entropy of order $\lambda$,  offers an alternative generalization motivated by non-extensive systems, those exhibiting long-range dependencies, memory effects, or multifractal behavior. Unlike Rényi, Tsallis entropy does not obey the additivity property under independent systems, which makes it particularly suitable for modeling complex spatial structures and textured regions in SAR images.
...
\end{equation}
As $\lambda \to 1$, $R_\lambda(Z)$ reduces to $H(Z)$.

Using&nbsp;\eqref{E:entropy-R}, we derive closed-form expressions for the Rényi entropy of the $\Gamma_{\mathrm{SAR}}$ and the $\mathcal{G}^0_I$ distributions.

\textbf{Homogeneous case.} We find
\begin{multline}
\label{eq-GammaSAR-R}
...
\textbf{Textured case.} For the $\mathcal{G}^0_I$ distribution, we obtain:
\begin{multline}  
\label{eq-HGI0-R}  
R_\lambda\bigl(\mathcal{G}^0_I(\mu, \alpha, L)\bigr) = R_\lambda\bigl(\Gamma_{\mathrm{SAR}}(L, \mu)\bigr) \;+ \;\ln(-1 - \alpha)  
...
\end{multline}
 The derivation of these results are proven in the Appendix&nbsp;\ref{app:A2}.
...
<!-- Fig.&nbsp;\ref{fig-convergence-renyi} illustrates this behaviour: as $\alpha$ decreases the colored curves of $R_\lambda(\mathcal{G}^0_I)$ converge towards the solid black curve of $R_\lambda(\Gamma_{\mathrm{SAR}})$, confirming the limiting behavior. -->
...
```{r fig-shannon-convergence-s, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="hbt", out.width="95%", fig.cap="Shannon entropy $H(\\mathcal{G}^0_I)$ converges to $H(\\Gamma_{\\mathrm{SAR}})$ as $\\alpha$ decreases.", fig.width=7, fig.height=3.2}
...
plot_shannon <- function(L) {
  ...
  ggplot() +
    geom_line(data = Entropy_gamma, aes(x = mu, y = Entropy_Gamma), color = "black", linewidth = 1.2) +
    geom_line(data = muEntropy.molten, aes(x = mu, y = value, color = alpha), linetype = "longdash", linewidth = 0.9, alpha=.7) + #linewidth = 2, alpha=.7
...
```{r fig-renyi-convergence-r, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="hbt", out.width="95%", fig.cap="Rényi entropy $R_{\\lambda}(\\mathcal{G}^0_I)$ converges to $R_{\\lambda}(\\Gamma_{\\mathrm{SAR}})$ as $\\alpha$ decreases.", fig.width=7, fig.height=3.2}
...
ggplot() +
  geom_line(data = Entropy_gamma, aes(x = mu, y = Entropy_Gamma), color = "black", linewidth = 1.2) +
  geom_line(data = muEntropy.molten, aes(x = mu, y = value, color = alpha), linetype = "longdash", linewidth = 0.9, alpha=.7) +
...
```{r fig-tsallis-convergence-t, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="H", out.width="95%", fig.cap="Tsallis entropy $T_{\\lambda}(\\mathcal{G}^0_I)$ converges to $T_{\\lambda}(\\Gamma_{\\mathrm{SAR}})$ as $\\alpha$ decreases.", fig.width=7, fig.height=3.2}
# fig.fullwidth = TRUE, out.width="95%"
...
The definition of Tsallis entropy is given in Equation \eqref{eq:tsallis}, this expression can be reformulated using the quantile function $Q(p)=F^{-1}(p)$, defined for $p\in(0,1)$. Since $f(Q(p)) = Q'(p)$, the expectation in \eqref{eq:tsallis} becomes
$$
...
$$
   \tilde f_{i,m}\;=\;\frac{n}{2m}\,D_{i,m}^{-1}.
$$
...
Approximating the integral in \eqref{eq:Tsallis-quantile} with a Riemann sum centered at $t_i\approx\tfrac{i}{n}$ and replacing $Q'(t)$ with the estimator in \eqref{eq:density-spacing}, we obtain the nonparametric plug-in estimator
\begin{align}
