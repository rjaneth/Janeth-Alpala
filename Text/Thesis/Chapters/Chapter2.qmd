...
$$
f_{\mathcal{G}^0_I}\big(z;  \alpha, \mu, L\big)
\longrightarrow 
...
\begin{equation}
  \label{eq:entropy-sh}
  H(Z) =   -\mathbb{E}\bigl[ \ln f(z)\bigr]=-\int_{\mathcal{B}} f(z)\ln f(z)\, \mathrm{d}z,
\end{equation}
where $f(\cdot)$ is the pdf that characterizes the distribution of the real-valued random variable $Z$ and $\mathcal{B} \subseteq \mathbb{R}$ denotes the support of $f(z)$.
...
\begin{equation}
\label{eq:gamma-sh1}
H\bigl(\Gamma_{\mathrm{SAR}}(\mu, L)\bigr) = L - \ln L + \ln\Gamma(L) + (1 - L)\psi^{(0)}(L) + \ln \mu,
\end{equation}
...
\begin{multline}
\label{eq:GIO-Sh1}
H\bigl(\mathcal{G}^0_I(\alpha, \mu, L)\bigr) =H\bigl(\Gamma_{\mathrm{SAR}}(\mu, L)\bigr)\;+\;
...
\vspace{-15pt}
$$
\lim_{\alpha \to -\infty} H\bigl(\mathcal{G}^0_I( \alpha, \mu, L)\bigr) = H\bigl(\Gamma_{\mathrm{SAR}}(\mu, L)\bigr).
$$ 
Appendix A provides a proof of this limiting behavior.
...
\begin{equation}
\label{E:entropy-R}
R_\lambda(Z)  = \frac{1}{1-\lambda }\ln \Bigl[ \mathbb{E}\Bigl( f^{\lambda - 1}(Z)\Bigr) \Bigr] =\frac{1}{1 - \lambda} \ln \int_{\mathcal{B}} [f(z)]^\lambda \,\mathrm{d}z,
\end{equation}
...
\textbf{Homogeneous case.} Let $Z\sim\Gamma_{\mathrm{SAR}}(\mu, L)$ with pdf given in \eqref{E:gamma1}. We compute
$$
I = \int_{0}^{\infty}
    \bigl[f_{\Gamma_{\mathrm{SAR}}}(z;\mu, L)\bigr]^{\lambda}
    \,\mathrm{d}z
...
\begin{equation}
  \label{eq-GammaSAR-R}
  \boxed{
  R_{\lambda}\bigl(\Gamma_{\mathrm{SAR}}(\mu, L)\bigr)
  = \ln\mu - \ln L
+ \frac{1}{1-\lambda}
\Bigl[
-\lambda\,\ln\Gamma(L)
...
$$
I
= \int_{0}^{\infty}\bigl[f_{\mathcal{G}^0_I}(z;\alpha,\gamma,L)\bigr]^{\lambda}\,\mathrm{d}z
= C^{\lambda}
  \int_{0}^{\infty}
    \frac{z^{\lambda(L-1)}}{(\gamma+Lz)^{\,\lambda(L-\alpha)}}\,\mathrm{d}z,
\qquad
...
$$
I \;=\;\int_{0}^{\infty}
        \bigl[f_{\mathcal{G}^0_I}(z;\alpha,\gamma,L)\bigr]^{\lambda}\,\mathrm{d}z
    \;=\;C^{\lambda}\!
        \int_{0}^{\infty}\!
          \frac{z^{\lambda(L-1)}}{(\gamma+Lz)^{\,\lambda(L-\alpha)}}\,\mathrm{d}z,
$$
With the change of variable $t=Lz/\gamma$ ($\,\mathrm{d}z=\gamma\,\,\mathrm{d}t/L$) we obtain:
$$
I
= C^{\lambda}\,
  \frac{\gamma^{\,1+\lambda(\alpha-1)}}{L^{\,1+\lambda(L-1)}}
  \int_{0}^{\infty}
    \frac{t^{\lambda(L-1)}}{(1+t)^{\,\lambda(L-\alpha)}}\,\mathrm{d}t.
$$
Using the Beta–function identity
$$
\int_{0}^{\infty}\frac{t^{a-1}}{(1+t)^{a+b}}\,\mathrm{d}t
= B(a,b),\;\;
a=\lambda(L-1)+1,\;\;
b=\lambda(-\alpha+1)-1,
$$
...
\end{multline*}
This expression extends the Rényi entropy of the $\Gamma_{\mathrm{SAR}}(\mu, L)$ model \eqref{eq-GammaSAR-R} by including additional terms that depend on the texture parameter $\alpha$. Specifically:
\begingroup\small
\begin{equation}
\label{eq-HGI0-R}
\boxed{%
...
\vspace{-15pt}
$$
\lim_{\alpha \to -\infty} R_\lambda\bigl(\mathcal{G}^0_I( \alpha, \mu, L)\bigr) = R_\lambda\bigl(\Gamma_{\mathrm{SAR}}(L, \mu)\bigr).
$$ 
Appendix A provides proof of this limiting behavior.
...
= \frac{1}{\lambda - 1} \Bigl[ 1 - \int_{\mathcal{B}} \bigl(f(z)\bigr)^{\lambda} \,\mathrm{d}z \Bigr],
\end{equation}
where $\mathcal{B} \subseteq \mathbb{R}$ denotes the support of $f(z)$.
...
Let $Z\sim\Gamma_{\mathrm{SAR}}(\mu, L)$  with pdf given by \eqref{E:gamma1}. Define the integral:
\begin{equation*}
I
= \int_0^\infty \left[f_{\Gamma_{\text{SAR}}}(z;\mu, L)\right]^{\lambda} \,\mathrm{d}z.
\end{equation*}
...
= \left(\frac{L^L}{\Gamma(L)\,\mu^L}\right)^{\lambda}
  \int_0^\infty
     z^{\lambda(L-1)}
     \exp\left(-\frac{\lambda L}{\mu} z\right)\,\mathrm{d}z.
\end{equation*}
Using the Gamma integral identity
\begin{align*}
\int_{0}^{\infty} x^{p-1}e^{-q x}\,\mathrm{d}x
...
\end{equation}
\endgroup
\textbf{Textured case.} Starting from the Tsallis definition in \eqref{eq:tsallis} and the pdf of $Z\sim\mathcal{G}^0_I(\alpha,\gamma,L)$ in \eqref{E:gi01}, define:
\begin{equation*}
...
\label{eq:J_initial}
\end{equation*}
Introducing the change of variables $t = \frac{Lz}{\gamma}$ (so that $z = \frac{\gamma t}{L}$ and $\mathrm{d}z = \frac{\gamma}{L}\,\mathrm{d}t$), we obtain
\begin{align}
I &= \left[ \frac{L^L \Gamma(L - \alpha)}{\gamma^\alpha \Gamma(-\alpha) \Gamma(L)} \right]^\lambda 
\frac{\gamma}{L} \int_0^\infty \frac{\left( \frac{\gamma t}{L} \right)^{\lambda(L-1)}}
{[\gamma(1 + t)]^{\lambda(L - \alpha)}}\,\mathrm{d}t\nonumber \\
...
\begin{equation*}
\int_0^\infty \frac{t^{a-1}}{(1+t)^{a+b}}\,\mathrm{d}z = B(a,b) = \frac{\Gamma(a)\Gamma(b)}{\Gamma(a+b)},
\end{equation*}
for parameters
\begin{equation*}
...
\biggr\}.
\end{equation*}
This expression extends the Tsallis entropy of the $\Gamma_{\mathrm{SAR}}(\mu, L)$ model \eqref{eq:gammasar-Tsallis} by including additional terms that depend on the texture parameter $\alpha$. It can be written in an exponential-logarithmic form as:
\begingroup\small
\begin{equation}
\label{eq:GI0-Tsallis}
\boxed{%
\begin{aligned}
  T_\lambda\bigl(\mathcal{G}^0_I(\mu,\alpha,L)\bigr) = T_\lambda\bigl(\Gamma_{\mathrm{SAR}}(\mu, L)\bigr)\;+\;\frac{1}{\lambda-1}\,
  \exp\Bigl[
  (1-\lambda)\ln\mu
  +(\lambda-1)\ln L
  +\ln\Gamma\bigl(\lambda(L-1)+1\bigr)
...
$\alpha\to-\infty$, validating the decomposition, i.e.,
$$
\lim_{\alpha \to -\infty} T_\lambda\bigl(\mathcal{G}^0_I( \alpha, \mu, L)\bigr) = T_\lambda\bigl(\Gamma_{\mathrm{SAR}}(L, \mu)\bigr).
$$ 
Appendix A provides proof of this limiting behavior.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    entropy/.style={
      ...
      minimum width=2.6cm, align=center, font=\sffamily
    },
    property/.style={
      rectangle, draw=orange!60, fill=orange!10, thick,
      minimum width=2.6cm, align=center, font=\sffamily
    },
  ...
  \node[property, below=1.6cm of renyi] (decomp) {Entropy Decomposition\\[0.5ex]
    $\text{Entropy}\bigl(\mathcal{G}^{0}_{I}\bigr)
    \;=\;
    \text{Entropy}\bigl(\Gamma_{\mathrm{SAR}}\bigr)
    \;+\;
  ...
  % --- OJO: espacio tras \\ para que NO sea \(
  \node[property, below left=1.0cm of decomp, align=center] (baseline)
    {Homogeneous Baseline\\ ($\Gamma_{\text{SAR}}$)};
  \node[property, below right=1.0cm of decomp, align=center] (texture)
    {Texture-Induced Excess\\ ($\Delta_{\alpha}$)};

  \node[property, below=1.5cm of texture, align=center] (limits)
    {Vanishing Excess\\ $\Delta_\alpha \to 0$ as $\alpha\to -\infty$};
  \node[property, below=1.5cm of baseline, align=center] (test)
    {Statistical Test\\ $\Delta_\alpha \neq 0?$};
...
Figures&nbsp;\ref{fig-shannon-convergence-s}--\ref{fig-tsallis-convergence-t} illustrate the Shannon ($H$), Rényi ($R_\lambda$), and Tsallis ($T_\lambda$) entropies for the $\mathcal{G}^0_I$ model under different texture levels ($\alpha$) and numbers of looks ($L$).  In every case the coloured dashed curves converge to the solid black baseline of the homogeneous $\Gamma_{\mathrm{SAR}}$ distribution as $\alpha\to-\infty$, confirming the limiting behavior.

For the Shannon entropy (Figure&nbsp;\ref{fig-shannon-convergence-s}), when $L = 5$ the entropy curves for different $\alpha$ values are more concentrated. This is due to the higher speckle noise present in a low number of looks, which dominates the uncertainty and masks texture effects. When $L = 18$, speckle is reduced and the entropy better reflects texture variation, resulting in more separated curves.

In the case of Rényi entropy (Figure&nbsp;\ref{fig-renyi-convergence-r}), for $\lambda = 0.6$ and $\lambda = 0.9$, the general shape and separation of the curves are similar. However, increasing $L$ leads to a slight decrease in entropy values across all $\alpha$, consistent with reduced speckle. The convergence pattern toward the Gamma baseline remains stable in both settings.

For Tsallis entropy (Figure&nbsp;\ref{fig-tsallis-convergence-t}), the separation between curves is most noticeable for $\lambda = 0.6$ and $L = 5$, where texture has more influence. When $\lambda$ increases to $0.9$ or when $L$ is increased, the curves become more compact and approach the Gamma entropy faster, indicating lower sensitivity to texture variation and confirming that $\lambda$ controls the influence of the tail behavior.

```{r fig-shannon-convergence-s,  eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="hbt", out.width="98%", fig.cap="Shannon entropy $H(\\mathcal{G}^0_I)$ converges to $H(\\Gamma_{\\mathrm{SAR}})$ as $\\alpha$ decreases.", fig.width=8, fig.height=3.9} 
...
```{r fig-renyi-convergence-r,  eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="hbt", out.width="98%", fig.cap="Rényi entropy $R_{\\lambda}(\\mathcal{G}^0_I)$ converges to $R_{\\lambda}(\\Gamma_{\\mathrm{SAR}})$ as $\\alpha$ decreases.", fig.width=8, fig.height=3.9}
...
```{r fig-tsallis-convergence-t,  eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="H", out.width="98%", fig.cap="Tsallis entropy $T_{\\lambda}(\\mathcal{G}^0_I)$ converges to $T_{\\lambda}(\\Gamma_{\\mathrm{SAR}})$ as $\\alpha$ decreases.", fig.width=8, fig.height=3.9}
# fig.fullwidth = TRUE, out.width="95%"
...
\caption{\label{fig-shannon-convergence-s}Shannon entropy
\(H(\mathcal{G}^0_I)\) converges to \(H(\Gamma_{\mathrm{SAR}})\) as
\(\alpha\) decreases.}

\centering{

\includegraphics[width=0.98\linewidth,height=\textheight,keepaspectratio]{Figures/fig-shannon-convergence-s-1.pdf}
...
\caption{\label{fig-renyi-convergence-r}Rényi entropy
\(R_{\lambda}(\mathcal{G}^0_I)\) converges to
\(R_{\lambda}(\Gamma_{\mathrm{SAR}})\) as \(\alpha\) decreases.}

\centering{

\includegraphics[width=0.98\linewidth,height=\textheight,keepaspectratio]{Figures/fig-renyi-convergence-r-1.pdf}
...
\caption{\label{fig-tsallis-convergence-t}Tsallis entropy
\(T_{\lambda}(\mathcal{G}^0_I)\) converges to
\(T_{\lambda}(\Gamma_{\mathrm{SAR}})\) as \(\alpha\) decreases.}

\centering{

\includegraphics[width=0.98\linewidth,height=\textheight,keepaspectratio]{Figures/fig-tsallis-convergence-t-1.pdf}
...
Since $F(z)$ is monotonic, $Q(p)$ is differentiable wherever $f(z)$ is positive, with derivative $Q'(p) = \frac{d}{\mathrm{d}p}F^{-1}(p)$. By the inverse function theorem, $Q'(p) = \frac{1}{f(Q(p))}$.

Starting from the definition of Shannon entropy in Equation \eqref{eq:entropy-sh}, a change of variables $p = F(z)$ can be applied. When $z = Q(p)$, it follows that $\mathrm{d}z = Q'(p)\,\mathrm{d}p$. Substituting into the integral:
\begin{align*}
  H(Z)
  &= -\int_{0}^{1} f(Q(p))\ln f(Q(p))\cdot Q'(p)\,\mathrm{d}p \\
  &= -\int_{0}^{1} \ln f(Q(p))\,\mathrm{d}p.
\end{align*}

Now using $f(Q(p)) = 1/Q'(p)$, we have $\ln f(Q(p)) = -\ln Q'(p)$. Thus:
\begin{equation}
  H(Z) = \int_{0}^{1} \ln Q'(p)\,\mathrm{d}p = \int_{0}^{1} \ln \left(\frac{d}{\mathrm{d}p}F^{-1}(p)\right)\,\mathrm{d}p.
\end{equation}
...
By taking logarithms and averaging over $i$, Vasicek’s estimator becomes:
\begin{equation}
\label{E:Vasi}
  \widehat{H}_{\text{V}}(\bm{Z}) = \frac{1}{n} \sum_{i=1}^{n} \ln\left[ \frac{n}{2m}(Z_{(i+m)} - Z_{(i-m)}) \right].
\end{equation}

Vasicek&nbsp;[-@vasicek1976test] showed that under regularity conditions ($m \to \infty$, $m/n \to 0$), $\widehat{H}_{\text{V}} \to H(f)$ in probability. 

Nonparametric estimators using order statistics and spacings, such as Vasicek’s m-spacing estimator, provide a flexible alternative to model-based entropy estimation. By converting the entropy integral into a quantile-based form (see Appendix B), these estimators offer robust, consistent estimation of Shannon entropy without requiring explicit density estimation.
...
\begin{align*}
\int f^\lambda(z)\,\mathrm{d}z
&= \int_0^1 [f(Q(p))]^\lambda \cdot Q'(p)\,\mathrm{d}p \\
&= \int_0^1 [Q'(p)]^{1 - \lambda}\,\mathrm{d}p.
\end{align*}
Substituting this into the original expression gives the quantile-based representation:
\begin{equation}
R_\lambda(Z) = \frac{1}{1 - \lambda} \ln \left( \int_0^1 [Q'(p)]^{1 - \lambda}\,\mathrm{d}p \right).
...
A detailed derivation of this estimator, including the quantile-based transformation and spacing approximation, is provided in Appendix B.
