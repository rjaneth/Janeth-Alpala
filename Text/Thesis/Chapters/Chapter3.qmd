...
For the Rényi estimator (see Figure&nbsp;\ref{fig-optimal_order}), $\lambda = 0.9$ minimizes the MSE while keeping the bias low, achieving a favorable bias-variance trade-off. Although $\lambda = 0.85$ yields slightly lower bias, it produces a higher MSE, supporting the choice of $\lambda = 0.9$ as optimal.

Similarly, for the Tsallis estimator (Figure&nbsp;\ref{fig-optimal_order-tsallis}), $\lambda = 0.85$ results in the lowest MSE with moderate bias, making it the most suitable choice.

The estimators remained numerically stable and converged smoothly to Shannon entropy as $\lambda \to 1$. 
Outside this range ($\lambda < 0.7$ or $\lambda > 1$), bias and MSE increased, indicating reduced numerical precision. 
These results confirm that the selected $\lambda$ values ensure stable and reliable entropy estimates under typical SAR conditions.

```{r fig-optimal_order, eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE, warning=FALSE,  out.width="70%",  fig.pos="htb", fig.cap="Bias and MSE as a function of $\\lambda$, for the Rényi entropy estimator, with $n = 49$, $L = 5$.", fig.width=5, fig.height=3.0}
#fig.show="hide", 
# eval=FALSE,  # 
...
```{r fig-optimal_order-tsallis,  eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE, warning=FALSE,  out.width="70%",  fig.pos="htb", fig.cap="Bias and MSE as a function of $\\lambda$, for the Tsallis entropy estimator, with $n = 49$, $L = 5$.", fig.width=5, fig.height=3.0}
#fig.show="hide", 
# eval=FALSE,  # 
...
\caption{\label{fig-optimal_order}Bias and MSE as a function of
\(\lambda\), for the Rényi entropy estimator, with \(n = 49\),
\(L = 5\).}

\centering{

\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{Figures/fig-optimal_order-1.pdf}
...
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{Figures/fig-optimal_order-tsallis-1.pdf}
...
```{r fig-bias_mse_rm1,   eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE, warning=FALSE,  out.width="65%",  fig.pos="H",  fig.cap="Bias and MSE of the Rényi entropy estimators for $\\Gamma_{\\text{SAR}}$, with $L=5$", fig.width=5.5, fig.height=3.5}
...
```{r Simulated_data_bias_tt,  eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE }
set.seed(1234567890, kind = "Mersenne-Twister")
#cache = TRUE, autodep = TRUE
...
```{r fig-bias_mse_Tsallis,  eval=FALSE,  include=FALSE,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="65%",  fig.pos="htb",  fig.cap="Bias and MSE of the Tsallis entropy estimators for $\\Gamma_{\\text{SAR}}$, with $L=5$", fig.width=5.5, fig.height=3.6}
...
\caption{\label{fig-bias_mse_rm1}Bias and MSE of the Rényi entropy
estimators for \(\Gamma_{\text{SAR}}\), with \(L=5\)}

\centering{

\includegraphics[width=0.65\linewidth,height=\textheight,keepaspectratio]{Figures/fig-bias_mse_rm1-1.pdf}
...
\includegraphics[width=0.65\linewidth,height=\textheight,keepaspectratio]{Figures/fig-bias_mse_Tsallis-1.pdf}
...
In particular, at $n = 49$, both estimators exhibit low bias and MSE, confirming their robustness in moderate sample regimes and supporting their use in practical applications.
...
An important advantage shared by all three proposed test statistics (\eqref{eq-test-sh}, \eqref{eq-test-renyi} and \eqref{eq-test-tsallis}) is that they avoid the need to estimate parameters from the alternative $\mathcal{G}^0_I$ distribution, such as the roughness parameter $\alpha$. 
This leads to a simpler, more interpretable, and statistically grounded testing procedure.

Additionally, the tests are computationally efficient and suitable for practical applications, including large-scale image processing. 
This robustness is especially evident in small sample sizes, where bootstrap correction improves the reliability of the entropy estimates and stabilizes the resulting $p$-values used for decision-making.

\subsection{Empirical Behaviour of the Three Statistics} 

Figures&nbsp;\ref{fig-densities-shannon}-\ref{fig-densities-tsallis4} display the empirical distributions of the three entropy-based statistics:Shannon $S_{\widetilde{H}}(\mathbf Z;L)$, Rényi $S_{\widetilde{R}_{\lambda}}(\mathbf Z;L)$ (with $\lambda=0.9$) and Tsallis $S_{\widetilde{T}_{\lambda}}(\mathbf Z;L)$ ($\lambda=0.85$) obtained from $10^{4}$ Monte Carlo replications of $\Gamma_{\text{SAR}}$ data for window sizes $n\in\{49,81,121\}$ and looks $L\in\{5,18\}$.  
...
```{r Simulated_density-shannon, eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE}
set.seed(1234567890, kind = "Mersenne-Twister")
...
```{r fig-densities-shannon, eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE, warning=FALSE, out.width="90%", fig.pos="hbt", fig.cap="Empirical densities of $S_{\\widetilde{H}_{AO}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$.", fig.width=9, fig.height=4.0}
...
```{r Simulated_densityR, eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE}
set.seed(1234567890, kind = "Mersenne-Twister")
...
```{r fig-densities, eval=FALSE,  include=FALSE,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="90%", fig.pos="hbt",  fig.cap="Empirical densities of $S_{\\widetilde{R}_{\\lambda}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$, with $\\lambda=0.9$.", fig.width=9, fig.height=4.0}
#fig.show="hide", 
# eval=FALSE,  #
...
```{r Simulated_density-tsallis2, eval=FALSE,  include=FALSE, echo=FALSE, message=FALSE}
set.seed(1234567890, kind = "Mersenne-Twister")
...
```{r fig-densities-tsallis4, eval=FALSE,  include=FALSE,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="90%", fig.pos="H",  fig.cap="Empirical densities of $S_{\\widetilde{T}_{\\lambda}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$, with $\\lambda=0.85$.", fig.width=9, fig.height=4.0}
#fig.show="hide", 
# eval=FALSE,  #
...
\caption{\label{fig-densities-shannon}Empirical densities of
\(S_{\widetilde{H}_{AO}}(\bm{Z}; L)\) under \(\mathcal{H}_0\).}

\centering{

\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{Figures/fig-densities-shannon-1.pdf}
...
\caption{\label{fig-densities}Empirical densities of
\(S_{\widetilde{R}_{\lambda}}(\bm{Z}; L)\) under \(\mathcal{H}_0\), with
\(\lambda=0.9\).}

\centering{

\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{Figures/fig-densities-1.pdf}
...
The observed Type&nbsp;I error rates for the three entropy-based tests were consistently close to the nominal levels ($0.01$, $0.05$, $0.10$), thereby confirming the validity and proper calibration of the procedures. These results are summarized in Tables&nbsp;[-@tbl-table_size_power_shannon]--[-@tbl-size-power-tsallis], and are visualized in the top panel of&nbsp;Figure&nbsp;\ref{fig-compare_size_power_plot0}.
...
The results are shown in Tables&nbsp;[-@tbl-table_size_power_shannon]--[-@tbl-size-power-tsallis], with graphical representations provided in the lower panel of&nbsp;Figure&nbsp;\ref{fig-compare_size_power_plot0}.
...
```{r}
#| label: tbl-table_size_power_shannon
#| echo: false
#| message: false
#| warning: false
#| results: 'asis'
#| tbl-cap: 'Size and Power of the $S_{\widetilde{H}}(\bm{Z})$ test statistic (Shannon)'
#| tbl-pos: 'H'
suppressMessages({
  suppressWarnings({
    load("./Data/type_I_results-shannon.Rdata")
...
\renewcommand{\arraystretch}{0.9}
```{r}
#| label: tbl-size-power-renyi
#| echo: false
#| message: false
#| warning: false
#| results: 'asis'
#| tbl-cap: 'Size and Power of the $S_{\widetilde{R}_{\lambda}}(\bm{Z})$ test statistic (Rényi)'
#| tbl-pos: 'H'
suppressMessages({
  suppressWarnings({
...
```{r}
#| label: tbl-size-power-tsallis
#| echo: false
#| message: false
#| warning: false
#| results: 'asis'
#| tbl-cap: 'Size and Power of the $S_{\\widetilde{T}_{\\lambda}}(\\bm{Z})$ test statistic (Tsallis)'
#| tbl-pos: 'H'
suppressMessages({
  suppressWarnings({
...
```{r fig-compare_size_power_plot0,  eval=FALSE,  include=FALSE, echo=FALSE, fig.width=10, fig.height=12, fig.pos="H", fig.cap="Comparative performance of test statistics based on Shannon, Rényi and Tsallis entropies. Size and Power for different sample sizes and $L$ values."}
...
\caption{\label{fig-compare_size_power_plot0}Comparative performance of
test statistics based on Shannon, Rényi and Tsallis entropies. Size and
Power for different sample sizes and \(L\) values.}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{Figures/fig-compare_size_power_plot0-1.pdf}}
...
## FIXED AND ADAPTIVE WINDOWS

This section outlines the two local windowing strategies that will be used in the next chapter to evaluate the proposed entropy-based tests on both simulated and real SAR intensity data. 

### Fixed Sliding Windows

The first strategy uses a fixed sliding window of size $7 \times 7$ pixels. 
For each window, the test statistics $S_{\widetilde{H}_{\text{AO}}}$, $S_{\widetilde{R}_{\lambda}}$, and $S_{\widetilde{T}_{\lambda}}$ will be computed, and the associated $p$-values derived. 
The $p$-value measures evidence against the null hypothesis of homogeneity (fully developed speckle under the $\Gamma_{\text{SAR}}$ model). High values (e.g., $p>0.05$) will be interpreted as no significant deviation from homogeneity, whereas low $p$-values (e.g., $p<0.05$) will be taken as *evidence of heterogeneity*. These $p$-values will later be visualized as maps to enable spatial assessment.



### Adaptive Windows 

The second strategy, used only with the Tsallis-entropy test $S_{\widetilde{T}_{\lambda}}$, builds on the adaptive windowing algorithm by Park et al. [-@Park1999]. The analysis window size is adjusted according to the local homogeneity of the scene. 
For each pixel $(i,j)$, let $w_{ij}$ be the window centered at $(i,j)$ with radius $N_{ij}$, defined as  
$w_{ij} = \{(i',j') \,/\, i-N_{ij} \le i' \le i+N_{ij} \,\wedge\, j-N_{ij} \le j' \le j+N_{ij}\}$.  
The border of this window is given by  
$b_{ij} = \{(i',j') \in w_{ij} \,/\, i' = i-N_{ij} \,\vee\, i' = i+N_{ij} \,\vee\, j' = j-N_{ij} \,\vee\, j' = j+N_{ij}\}$.

The coefficient of variation is computed over the border of the current window as $C_{ij} =\frac{\sigma_{ij}}{\mu_{ij}}$, where the mean $\mu_{ij}$ and the standard deviation $\sigma_{ij}$ are calculated using only the border pixels $b_{ij}$ of $w_{ij}$.
...
\end{cases}
$$
where $N_{\max}$ and $N_{\min}$ denote the maximum and minimum allowed window radius, respectively. Since $W_{ij}=2N_{ij}+1$, these bounds on the radius induce the corresponding minimum and maximum window sides $W_{\min}=2N_{\min}+1$ and $W_{\max}=2N_{\max}+1$. Hence, windows *grow* in homogeneous regions (stabilizing estimates) and *shrink* in heterogeneous areas (preserving edges and avoiding class mixing). 
...
In the experiments presented in the next chapter, the Tsallis-based test statistic $S_{\widetilde{T}{\lambda}}$ will be applied to both simulated and real SAR images using adaptive windows with side lengths ranging from $5\times5$ ($W_{\min}=5$) to $11\times11$ ($W_{\max}=11$), with the smoothing parameter set to $\eta = 3$. For comparison, a fixed $7\times7$ window configuration will also be evaluated.
