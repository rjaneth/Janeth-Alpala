...
Figure \ref{fig:Plot_bias_mse-sh} presents the bias and MSE for each of the non-parametric estimators of Shannon entropy. The results are summarized in @tbl-out.
...
```{r Table_estimators_shannon2, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
#| label: tbl-out
...
\end{equation}
<!-- This expression follows from the classical bias correction formula: -->
<!-- \begin{equation} -->
<!-- \text{Bias-corrected estimate} = \text{Original estimate} - \text{Bias estimate}. -->
<!-- \label{eq:bias_correction_principle} -->
<!-- \end{equation} -->
...
<!-- <!-- # ```{r Simulated_comparative_time6, eval=FALSE, echo=FALSE} --> 
...
```{r Table_statistic_time_6,  echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
#| label: tbl-out-time
...
<!-- ```{r Simulated_data_bias_shannon1, echo=FALSE, message=FALSE} -->
<!-- set.seed(1234567890) -->

<!-- file_name <- "./Data/results_shannon_v1.Rdata" -->

<!-- if (file.exists(file_name)) { -->
<!--   load(file_name) -->
<!--   message("Loaded existing results from results_shannon_v1.Rdata") -->
<!-- } else { -->
<!--   sample_sizes <- c(9, 25, 49, 81, 121) -->
<!--   R <- 500 -->
<!--   B <- 100 -->
...
<!--   estimators <- list( -->
<!--     "Shannon Estimator" = al_omari_1_estimator, -->
<!--     "Shannon Estimator Bootstrap" = bootstrap_al_omari_1_estimator -->
<!--   ) -->

<!--   results <- calculate_bias_mse_shannon(sample_sizes, R, B, mu, L, estimators) -->

<!--   save(results, file = file_name) -->
<!--   message("Simulaciones completadas y resultados guardados.") -->
<!-- } -->

<!-- ``` -->
<!-- ```{r fig-bias_mse_shannonm, echo=FALSE, message=FALSE, warning=FALSE, out.width="80%", fig.cap="Bias and MSE of the Shannon–Al-Omari entropy estimators for $\\Gamma_{\\text{SAR}}$.", fig.width=6, fig.height=5} -->
<!-- load("./Data/results_shannon_v1.Rdata") -->

<!-- estimators_to_plot <- c("Shannon Estimator", "Shannon Estimator Bootstrap") -->
<!-- latex_estimator_names <- c( -->
<!--   "Shannon Estimator" = expression("$\\widehat{italic(H)}$"), -->
<!--   "Shannon Estimator Bootstrap" = expression("$\\widetilde{italic(H)}$") -->
<!-- ) -->
<!-- selected_estimators_latex <- latex_estimator_names[estimators_to_plot] -->

<!-- combined_plot_shannon <- generate_plot_shannon(results, selected_estimators_latex, ncol = 1, nrow = 1) -->
...
```{r fig-densities-shannon, echo=FALSE, message=FALSE, warning=FALSE, out.width="80%", fig.pos="hbt", fig.cap="Empirical densities of $S_{\\widetilde{H}_{AO}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$.", fig.width=9, fig.height=4.0}
...
for (L in selected_L_values) {
 ...
 # scale_color_brewer(palette = "Dark2", name = "Sample Size") +
  ...
  #   scale_y_continuous(limits = y_limits, minor_breaks = NULL) +
  #   labs(
  #       x = expression("Test Statistic" ~ S[widetilde(italic(T))](italic(bold(Z))* ";" **phantom(" ")* italic(L))), 
  #       y = "Density"
  #   ) +
  #   ggtitle(bquote(italic(L) == .(L))) +
  #   theme(plot.title = element_text(hjust = 0.5, size = 16, margin = margin(b = 2)),
...
```{r fig-densities,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="80%", fig.pos="hbt",  fig.cap="Empirical densities of $S_{\\widetilde{R}_{\\lambda}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$, with $\\lambda=0.9$.", fig.width=9, fig.height=4.0}
#fig.show="hide", 
# eval=FALSE,  #
...
```{r fig-densities-tsallis4,  echo=FALSE, message=FALSE, warning=FALSE,  out.width="80%", fig.pos="H",  fig.cap="Empirical densities of $S_{\\widetilde{T}_{\\lambda}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$, with $\\lambda=0.85$.", fig.width=9, fig.height=4.0}
#fig.show="hide", 
# eval=FALSE,  #
...
\begin{tikzpicture}[
  ...
  \node[box] (raw) {1. Compute\\test statistic\\ $S(\bm{Z})$};
  \node[box,right=of raw] (std) {2. Standardize\\ $\varepsilon=(S-\hat\mu_S)/\hat\sigma_S$};
  \node[box,right=of std] (pval) {3. Compute \\ $2\Phi(-|\varepsilon|)$};
  \node[box,right=of pval] (dec) {4. Reject if\\ $p<0.05$};

  \draw[arrow] (raw)  -- (std);
  \draw[arrow] (std) -- (pval);
  \draw[arrow] (pval) -- (dec);
\end{tikzpicture}
    \caption{Workflow of the entropy‐based hypothesis test: compute statistic, standardize, derive $p$-value, and decision by threshold $0.05$.}
