...
\end{cases}
$$
where $N_{\max}$ and $N_{\min}$ denote the maximum and minimum allowed window size, respectively. Hence, windows *grow* in homogeneous regions (stabilizing estimates) and *shrink* in heterogeneous areas (preserving edges and avoiding class mixing). 
...
In the experiments presented in the next chapter, the Tsallis-based test statistic $S_{\widetilde{T}{\lambda}}$ will be applied to both simulated and real SAR images using adaptive windows with side lengths ranging from $5\times5$ ($N{\min}=5$) to $11\times11$ ($N_{\max}=11$), with the smoothing parameter set to $\eta = 3$. For comparison, a fixed $7\times7$ window configuration will also be evaluated.
