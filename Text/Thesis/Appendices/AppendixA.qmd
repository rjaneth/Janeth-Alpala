...
# APPENDIX A – LIMIT BEHAVIOR OF ENTROPY FUNCTIONS {.unnumbered .unlisted} 
\addcontentsline{toc}{chapter}{\protect\numberline{}APPENDIX A – LIMIT BEHAVIOR OF ENTROPY FUNCTIONS}
<!-- {#sec-appendix-A} -->


##  Limit Behavior of $H\bigl(\mathcal{G}^0_I\bigr)$ as $\alpha \to -\infty$ {.unlisted}

To verify that $H\bigl(\mathcal{G}^0_I( \alpha, \mu, L)\bigr)$ converges to $H\bigl(\Gamma_{\text{SAR}}(\mu , L)\bigr)$ as $\alpha \to -\infty$, we show that the additional terms in $H\bigl(\mathcal{G}^0_I\bigr)$ cancel in the limit.

The Shannon entropy for the $\mathcal{G}^0_I$ distribution is given by:
\begin{multline}
\label{eq:GIO-Sh}
H\bigl(\mathcal{G}^0_I( \alpha, \mu, L)\bigr) =H(\Gamma_{\text{SAR}}) +
\Bigl[ (L-\alpha) \psi^{(0)}(L-\alpha)-(1-\alpha)\psi^{(0)}(-\alpha)
+\ln (-1-\alpha) -\ln\Gamma(L-\alpha)\\ +\ln\Gamma(-\alpha)-L\Bigr].
\end{multline}
We aim to show that
$$
\lim_{\alpha \to -\infty} H\bigl(\mathcal{G}^0_I( \alpha, \mu, L)\bigr) = H\bigl(\Gamma_{\text{SAR}}(\mu , L)\bigr).
$$ 
...
## Limit Behavior of \texorpdfstring{$R_\lambda(\mathcal{G}^0_I)$}{Hl(GI0)} as \texorpdfstring{$\alpha \to -\infty$}{alpha->-∞} {.unlisted}


We want to show that 
$$
\lim_{\alpha \to -\infty}
R_\lambda\bigl(\mathcal{G}^0_I\bigr)( \alpha, \mu, L)
=
...
## Limit Behavior of \texorpdfstring{$T_\lambda(\mathcal{G}^0_I)$}{Hl(GI0)} as \texorpdfstring{$\alpha \to -\infty$}{alpha->-∞} {.unlisted}

We want to show that 
$$
\lim_{\alpha \to -\infty}
T_\lambda\bigl(\mathcal{G}^0_I\bigr)( \alpha, \mu, L)
=
...
# APPENDIX B – DERIVATIONS OF SPACING ESTIMATORS {.unnumbered .unlisted} 
\addcontentsline{toc}{chapter}{\protect\numberline{}APPENDIX B – DERIVATIONS OF SPACING ESTIMATORS}



This appendix mirrors the derivation given for Vasicek’s estimator of Shannon entropy, and the Tsallis  estimator.

## From Quantile Function to Vasicek Estimator {.unlisted}
...
## Tsallis Entropy and the Quantile Function  {.unlisted}
