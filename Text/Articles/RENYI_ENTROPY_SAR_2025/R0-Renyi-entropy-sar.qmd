...
\Bigr].
\end{multline}
These results are proven in the Appendix.
...
@fig-plot presents the Rényi entropy of the $\mathcal{G}^0_I$ model as a function of $\mu$ for different values of $\alpha$. 
As $\alpha \to -\infty$, it approaches the Rényi entropy of the $\Gamma_{\text{SAR}}$ law, in line with the fact that the latter is a limiting case of the former.
...
Such non-parametric estimation of $H(Z)$ has been widely studied using spacing-based estimators, which rely on differences between order statistics [@vasicek1976test; @Ebrahimi1994; @Wieczorkowski1999; @AlOmari2019]. 
...
A Monte Carlo study with $1000$ samples from $\Gamma_{\text{SAR}}$, $\mu = 1$ and $L=5$, using $B = 200$ replications, confirms that the bootstrap-corrected estimator $\widetilde{H}_{\lambda}$, with $\lambda = 0.9$, outperforms the original $\widehat{H}_{\lambda}$ by reducing both bias and MSE for small sample sizes, as shown in @fig-Plot_bias_msef3.
...
@fig-density_entropyR shows the empirical density of $S(\bm{Z}; L)$, based on $10^4$ replications of the $\Gamma_{\text{SAR}}$ model, with $\lambda=0.9$ and $L \in \{5,18\}$.
...
Vasicek&nbsp;[@vasicek1976test] proved that, for sufficiently large samples, $S(\bm{Z}; L)$ asymptotically follows a normal distribution:
\begin{equation*}
S(\bm{Z}; L) \overset{d}{\longrightarrow} \mathcal{N}(\mu_S,\,\sigma^{2}_S)\,,
\end{equation*}
where $d$ represents convergence in distribution. Here, $\mu_S  = \mathbb{E}[S(\bm{Z}; L)]$ and $\sigma^{2}_S = \text{Var}[S(\bm{Z}; L)]$ are the theoretical mean and variance of $S(\bm{Z}; L)$.

We define the standardized test statistic:
\begin{equation*}
\varepsilon = \frac{S(\bm{Z}; L) - \hat{\mu}_S}{\hat{\sigma}_S},
\end{equation*}
where $\hat{\mu}_S$ and $\hat{\sigma}_S$ are the empirical mean and standard deviation of $S(\bm{Z}; L)$, obtained by Monte Carlo simulations under the null hypothesis. 
...
In this section, we evaluate the proposed test statistic on SAR data to assess its effectiveness in detecting heterogeneity.
We applied the test to images covering urban, agricultural, and water regions. 
We analyzed images from London, United Kingdom; from the surroundings of Munich, Germany; and from  Dublin Port, Ireland, as shown in Figs.&nbsp;\ref{fig:london}(a), \ref{fig:munich}(a), and \ref{fig:dublin}(a).
...
We applied the test statistics $S_{\widetilde{H}_{\lambda}}(\bm{Z}; L)$ and  $S_{\widetilde{H}_{\text{AO}}}(\bm{Z}; L)$, based on Shannon entropy as described in&nbsp;[@Frery2024], using a local sliding window of size $7\times7$. 
...
Figs.&nbsp;\ref{fig:london}(c), \ref{fig:london}(e), \ref{fig:munich}(c), \ref{fig:munich}(e), \ref{fig:dublin}(c), and \ref{fig:dublin}(e) 
are binary maps at significance level of $0.05$ that 
...
The code is available at: <https://github.com/rjaneth/renyi-entropy> 
