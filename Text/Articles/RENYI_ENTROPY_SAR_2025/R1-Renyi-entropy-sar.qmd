...
abstract: |
  ...
  Experimental results demonstrate the robustness of the proposed method in distinguishing roughness patterns.
...
Fig.&nbsp;\ref{fig-convergence} illustrates this behaviour: as $\alpha$ decreases the coloured curves of $H_\lambda(\mathcal{G}^0_I)$ converge towards the solid black curve of $H_\lambda(\Gamma_{\mathrm{SAR}})$.
...
\textcolor{blue}{
We aim to determine the optimal order $\lambda$ for the Rényi entropy estimator using simulated samples of size $n = 49$, drawn from the $\Gamma_{\text{SAR}}$ distribution.
We computed the bias and the mean squared error (MSE) for vaying $\lambda$ with a Monte Carlo experiment.
We found that for $L > 1$, $\lambda = 0.9$ consistently yields the lowest MSE while maintaining a low bias, thus offering a favorable trade-off between bias and variance. 
As shown in Fig. \ref{fig-optimal_order}, $\lambda = 0.85$ produces slightly lower bias, but leads to higher MSE, supporting the choice of $\lambda = 0.9$.}
...
Following Refs.&nbsp;[@Frery2024;@Alpala2024], we refined the non-parametric entropy estimator $\widehat{H}_{\lambda}$ in&nbsp;\eqref{eq:est_R} with bootstrap, obtaining $\widetilde{H}_{\lambda}$:
\begin{equation*}
...
where $\widehat{\mu}_S$ and $\widehat{\sigma}_S$ are the empirical mean and standard deviation of $S_{\widetilde{H}_{\lambda}}(\bm{Z}; L)$, obtained by Monte Carlo simulations under the null hypothesis. 
...
High $p$-values, e.g., $p > 0.05$, indicate no evidence to reject homogeneity, while low values ($p < 0.05$) suggest statistically significant deviations and are interpreted as heterogeneity.
}
