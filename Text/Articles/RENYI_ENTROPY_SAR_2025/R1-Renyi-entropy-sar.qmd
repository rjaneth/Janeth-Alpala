...
\;\;\;\;\; +
\underbrace{\Delta_\alpha}_{\text{extra entropy caused by texture}},
$$
\textcolor{blue}{where the excess term $\Delta_\alpha$ depends on the roughness parameter.
When $\alpha\!\to\!-\infty$ (fully developed speckle) this excess tends to zero, so the heterogeneous model collapses to the homogeneous one in Equation \eqref{eq-HGammaSAR}. 
In practice, detecting heterogeneity is equivalent to deciding whether $\Delta_\alpha$ is significantly different from zero.}
...
Fig.&nbsp;\ref{fig-convergence} illustrates this behaviour: as $\alpha$ decreases the coloured curves of $H_\lambda(\mathcal{G}^0_I)$ converge towards the solid black curve of $H_\lambda(\Gamma_{\mathrm{SAR}})$, confirming the limiting behavior.
...
Such non-parametric estimation of $H(Z)$ has been widely studied using spacing-based estimators, which rely on differences between order statistics&nbsp;[@vasicek1976test;@Ebrahimi1994; @IbrahimAlOmari2014]. 
...
\textcolor{blue}{
We aim to determine the optimal order $\lambda$ for the Rényi entropy estimator based on numerical experiments using synthetic samples of size $n = 49$, drawn from the $\Gamma_{\text{SAR}}$ distribution $(L = 5,\ \mu = 1)$. A Monte Carlo simulation is used to evaluate the bias and the mean squared erro (MSE) across different values of $\lambda$.}
...
\end{figure}
```
Particularly, for $L = 1$, the optimal $\lambda$ tends to be higher, and we chose $\lambda = 3$ to achieve good results.
...
A Monte Carlo study with $1000$ replications for each sample size $n \in \{9, 25, 49, 81, 121\}$ from the $\Gamma_{\text{SAR}}$ ($L=5,\ \mu=1$) confirms that for $\lambda=0.9$, the bootstrap-corrected estimator $\widetilde{H}_{\lambda}$ ($B=200$) reduces both bias and MSE compared to the original $\widehat{H}_{\lambda}$, with significant improvements for small sample sizes, as shown in Fig. \ref{fig-bias_mse}.
...
## The Proposed Test
The test statistic quantifies the discrepancy between estimated and theoretical entropy under homogeneity. Since $L\geq1$ is known, we define the test statistic as follows:
\begin{multline}
\label{eq-test}
...
\;-\;
\underbrace{H_\lambda\bigl(\Gamma_{\mathrm{SAR}}\bigr)}_{\text{theoretical under } H_0},
$$
\textcolor{blue}{the difference between the estimated entropy and the expected value under homogeneity. Values close to zero indicate that the region behaves like fully developed speckle, while large positive values signal excess entropy and thus heterogeneity.
This formulation avoids estimating parameters of the $\mathcal{G}^0_I$ model such as $\alpha$, offering a simple, interpretable, and statistically grounded test. Moreover, the method remains effective for small samples, aided by the bootstrap bias correction.}

Fig. \ref{fig-densities} shows the empirical distribution of $S_{\widetilde{H}_{\lambda}}$ under $\mathcal{H}_0$, obtained from $10^4$ Monte Carlo simulations with varying sample sizes ($n \in {49,81,121}$), $\lambda=0.9$, and $L \in {5,18}$. The concentration around zero confirms the test's expected behavior, while the heavy tails indicate sensitivity to deviations, enabling the detection of subtle texture changes.
...
```{r fig-density_standardized,eval=FALSE,  echo=FALSE, message=FALSE, warning=FALSE, fig.show="hide", out.width="35%", fig.pos="hbt", fig.cap="Standardized empirical densities of $S_{\\widetilde{H}_{\\lambda}}(\\bm{Z}; L)$ under $\\mathcal{H}_0$.", fig.width=4, fig.height=2.5}
...
```
\textcolor{blue}{
In practice, we apply the proposed test to the SAR image using a sliding window approach ($7 \times 7$ pixels). For each window, we compute the test statistic under the null hypothesis of homogeneity and, assuming asymptotic normality, calculate a $p$-value that quantifies the statistical evidence against homogeneity in each local region. High $p$-values ($p > 0.05$) indicate no evidence to reject homogeneity, while low values ($p < 0.05$) suggest statistically significant deviations and are interpreted as heterogeneity.
}
...
Corresponding optical images (Figs.&nbsp;\ref{fig:london-optical}–\ref{fig:dublin-optical}) illustrate land cover context. Table&nbsp;\ref{tab:table_param} details the SAR acquisition parameters.
...
```
\textcolor{blue}{
For each SAR image, the number of looks $L$ was obtained from the product of azimuth and range looks provided in the metadata (SNAP). To validate this value, we estimated the equivalent number of looks (ENL) from manually selected homogeneous areas using the classical formula $\text{ENL} = 1/\widehat{\text{CV}}^2$, based on the coefficient of variation. The results were consistent with the metadata. We also verified that moderate deviations in $L$ did not significantly impact the outcome of the test, confirming the robustness of the proposed method.}


## Qualitative Inspection

The output consists of $p$-value maps and binary maps at a $0.05$ significance level. 
The $p$-value maps use a color gradient (Figs.&nbsp;\ref{fig:london-renyi},&nbsp;\ref{fig:london-shann},&nbsp;\ref{fig:munich-renyi},&nbsp;\ref{fig:munich-shann},&nbsp;\ref{fig:dublin-renyi},&nbsp;\ref{fig:dublin-shann}), where darker regions indicate areas with higher roughness, and lighter regions denote smoother, less textured surfaces. 
...
\begin{figure*}[hbt]
    ...
    \begin{subfigure}{0.179\textwidth}
        \includegraphics[width=\linewidth]{./Figures-R1/p-values_renyi-london_E.png}
   ...
   \hspace{0.00001\textwidth}
    \begin{subfigure}{0.18\textwidth}
        \includegraphics[width=\linewidth]{./Figures-R1/p-values_shannon-london_E.png}
...
\begin{subfigure}{0.145\textwidth}
    ...
    \hspace{0.00001\textwidth}
    \begin{subfigure}{0.178\textwidth}
        \includegraphics[width=\linewidth]{./Figures-R1/p-values_renyi-dublin_B.png}
    ...
    \hspace{0.00001\textwidth}
    \begin{subfigure}{0.178\textwidth}
        \includegraphics[width=\linewidth]{./Figures-R1/p-values_shannon-dublin_B.png}
...
Using the same ROIs as ground truth, we evaluated the continuous $p$-value maps by computing receiver operating characteristic (ROC) curves over all decision thresholds&nbsp;(Figs.&nbsp;\ref{ROC1-1}). Each curve plots the probability of detection (Pd) versus the probability of false alarm (PFA). Pd reflects the proportion of heterogeneous pixels correctly detected, while PFA represents the proportion of homogeneous pixels misclassified as heterogeneous.
...
\textcolor{blue}{Using real SAR data, we compared the Rényi-based test with a Shannon-based counterpart through $p$-value maps and binary decisions. Visual analysis showed greater sensitivity of the Rényi-based test to textural variations.
