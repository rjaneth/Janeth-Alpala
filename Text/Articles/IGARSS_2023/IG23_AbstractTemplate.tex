...
And, the Entropy is used for measuring the richness of data in enhanced images.
    ...
    %where $G$ is the number of grey level of the image’s histogram and $d(i)$ is the normalised frequency of occurrence of each grey level.
...
According to the research in~\cite{DeBorba2020}, various fusion methods were utilized to detect edge evidence. As an example, the results obtained by applying PCA and ROC methods to an AIRSAR L-band image of Flevoland are shown in Fig.~\ref{fig:2}.%(a)-(b).
