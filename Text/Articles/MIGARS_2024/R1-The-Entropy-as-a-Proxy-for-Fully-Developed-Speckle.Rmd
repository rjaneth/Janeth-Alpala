...
The empirical distributions of $\Gamma_{\text{SAR}}$ and $G_I^0$ entropies are shown in Fig.&nbsp;\ref{fig:Plot_empirical_gamma_and_gi0}. 
...
```{r Simulated_empirical_gamma_and_gi0, echo=FALSE, message=FALSE}
...
for (alpha_val in alpha1_values) {
  ...
  TestStatisticsList[[as.character(alpha_val)]] <- list(TestStatistics1 = TestStatistics1, TestStatistics2 = TestStatistics2)
}


save(TestStatisticsList, file = "./Data/data_empirical_entropy.RData")

```

```{r Plot_empirical_gamma_and_gi0, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center", fig.pos="hbt", fig.cap="Empirical distributions of $\\Gamma_{\\text{SAR}}$ and $G_I^0$ entropies as $\\mu=1$, $L=8$ and $\\alpha\\in\\{-1000, -20, -8, -3\\}$."}



load("./Data/data_empirical_entropy.Rdata")
plots_list <- list()

for (alpha_val in alpha1_values) {
  plot_data <- TestStatisticsList[[as.character(alpha_val)]]
  
  max_density1 <- max(density(plot_data$TestStatistics1$Test_Statistics)$y, na.rm = TRUE)
  max_density2 <- max(density(plot_data$TestStatistics2$Test_Statistics2)$y, na.rm = TRUE)
  
  plot <- ggplot(plot_data$TestStatistics1, aes(x = Test_Statistics)) +
    geom_line(aes(col = factor(Sample_Size), linetype = factor(Sample_Size)), stat = "density", linewidth = 1.0) +
    geom_line(data = plot_data$TestStatistics2, aes(x = Test_Statistics2), stat = "density", color = "blue", linetype = "dashed", linewidth = 1) +
    annotate("text", x = max(plot_data$TestStatistics1$Test_Statistics) + 0.1, y = max_density1,
             label = TeX("$\\Gamma_{\\tiny{SAR}}$"), vjust = 5, hjust = 5, color = "red") +
    annotate("text", x = max(plot_data$TestStatistics2$Test_Statistics2) + 0.1, y = max_density2,
...
$$
 \begin{cases}\mathcal{H}_0: \widetilde{H}(\bm{Z})-H(\bm{Z})=0 \\ 
  \mathcal{H}_1:\widetilde{H}(\bm{Z})-H(\bm{Z})\neq0 .\end{cases}
$$
...
Notably, small variance and near-zero skewness and kurtosis suggest limited spread, symmetry, and light tails. The coefficient of variation shows that as the sample size increases, variability with respect to the mean decreases, aligning with normal distribution expectations.
...
table_result_combined <- kbl(
  ...
  caption = "Descriptive analysis of $S(\\bm Z; L)$ for different values of $L$, and $\\mu=1$.",
  format = "latex",
  booktabs = TRUE,
  align = "lrrrrrrr",
  escape = FALSE,
...
We proceeded to explore the capabilities of the proposed test in terms of size and power. Under $\mathcal{H}_0$ the test statistic  is asymptotically distributed as $\mathcal{N}(0,\sigma_S^2))$, and therefore, the $p$-value of the observed test statistic is $2(1-\Phi(\varepsilon))$, where $\Phi$ is the standard Gaussian cumulative distribution function and 
$$
\varepsilon=\frac{\tilde{H}(\bm{Z})-H(\bm{Z})}{\sigma_S}.
$$
Nominal levels of $1\%$, $5\%$, and $10\%$ were considered. In reference to the size, $1000$ simulations have been used for various sample sizes with  satisfactory results. In all cases, the nominal level has been achieved. The power, in general, improves with the increase in the number of looks. The results are presented in Table \ref{tab:table_size_power}, providing a more detailed insight into the effectiveness of the test under different conditions.
...
calculate_p_value <- function(test_statistic, mu_W, sigma_W, alpha_nominal) {
  ...
  return(p_value < alpha_nominal)  # Return TRUE if p_value is less than alpha_nominal
}
...
# Combine the two datasets based on L and sample size
...
```{r Sim_empirical_test, echo=FALSE, message=FALSE, cache = TRUE, autodep = TRUE}
...
To assess the practical efficacy of the proposed test, we conducted experiments using real SAR images acquired by the Sentinel-1A system operating in the C band, with  HH polarization, and intensity format. The set of four SAR images, depicted in Fig.&nbsp;\ref{fig:Real_SAR_Images}(a)-(d) represent diverse scenes: a forested area in the Amazonas region, agricultural fields in Flevoland, and urban areas in Berlin and Ottawa, each with sizes of $512 \times 512$ pixels. 

We evaluate the test accuracy with small data portions. Ensuring resistance to contamination is crucial, especially in small samples with $5\times 5$, $7\times 7$, or $9\times 9$ windows that may contain data from areas with varying textures. Our assessment determines if the test can produce reasonable estimations, even when some data deviates from the assumed model.

Table&nbsp;\ref{tab:table_sar} shows the results for $5 \times 5$ windows, detailing the performance of the proposed test in accurately identifying the origin of the data. The metadata associated with these original SAR images includes a number of looks $L = 5$.


The openly accessible Sentinel-1 data used in this study was downloaded as Level-1 Ground Range Detected (GRD) products from the Copernicus Data Space Ecosystem (\url{https://dataspace.copernicus.eu}). This format, gaining popularity among operational and application users, containing only the co-polarized and cross-polarized backscatter intensities (VV, VH). 


```{r window, fig.align="center",  out.width = "80%", fig.cap=" Sketch of sliding window.", fig.show="hold",  fig.pos="hbt"}
knitr::include_graphics("../../../Figures/PDF/sliding2.pdf")
```



```{r Real_SAR_Images, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center", fig.cap='Real SAR images', fig.subcap=c('Amazonas', 'Flevoland','Ottawa', 'Berlin' ), out.width='.48\\linewidth',  fig.ncol=2}
knitr::include_graphics("../../../Figures/PNG/Brazil_Intensity_VV.png")
knitr::include_graphics("../../../Figures/PNG/Flevoland_Intensity_VV.png")
knitr::include_graphics("../../../Figures/PNG/Ottawa_Intensity_VV.png")
knitr::include_graphics("../../../Figures/PNG/Berlin_Intensity_VV.png")
```


```{r SAR_simulations, echo=FALSE, message=FALSE, cache = TRUE, autodep = TRUE}
#, cache = TRUE, autodep = TRUE
set.seed(1234567890, kind = "Mersenne-Twister")
...
x_Flevoland2 <- myread.ENVI(file='../../../Data/SAR/Flevoland2/Intensity_VV.img', headerfile='../../../Data/SAR/Flevoland2/Intensity_VV.hdr')
x_Ottawa <- myread.ENVI(file='../../../Data/SAR/Ottawa/Intensity_VV.img', headerfile='../../../Data/SAR/Ottawa/Intensity_VV.hdr')
...
centerRow_Brazil <- floor(nRows_Brazil / 2)
centerCol_Brazil <- floor(nCols_Brazil / 2)

roiSize_Brazil <- 5

# Coordinates of the ROI
roiStartRow_Brazil <- centerRow_Brazil - floor(roiSize_Brazil / 2) + 1
roiEndRow_Brazil <- roiStartRow_Brazil + roiSize_Brazil - 1

roiStartCol_Brazil <- centerCol_Brazil - floor(roiSize_Brazil / 2) + 1
roiEndCol_Brazil <- roiStartCol_Brazil + roiSize_Brazil - 1

# Extract the ROI
roi_Brazil <- x_Brazil[roiStartRow_Brazil:roiEndRow_Brazil, roiStartCol_Brazil:roiEndCol_Brazil]

mean_data_brasil <- mean(roi_Brazil)
Entropy_brasil <- correa_estimator(roi_Brazil)
entropia_estimada_boot_brasil <- bootstrap_correa_estimator(roi_Brazil, B)
True_entropy_brasil <- (log(mean(roi_Brazil)) + (L - log(L) + lgamma(L) + (1 - L) * digamma(L)))
Test_difference_brasil <- bootstrap_correa_estimator_log_mean(roi_Brazil, B) - ((L - log(L) + lgamma(L) + (1 - L) * digamma(L)))

# Berlin
nCols_Berlin = 512
nRows_Berlin = 512

centerRow_Berlin <- floor(nRows_Berlin / 2)
centerCol_Berlin <- floor(nCols_Berlin / 2)

roiSize_Berlin <- 5

# Calculate ROI coordinates
roiStartRow_Berlin <- centerRow_Berlin - floor(roiSize_Berlin / 2) + 1
roiEndRow_Berlin <- roiStartRow_Berlin + roiSize_Berlin - 1

roiStartCol_Berlin <- centerCol_Berlin - floor(roiSize_Berlin / 2) + 1
roiEndCol_Berlin <- roiStartCol_Berlin + roiSize_Berlin - 1

# Extract the ROI
roi_Berlin <- x_Berlin[roiStartRow_Berlin:roiEndRow_Berlin, roiStartCol_Berlin:roiEndCol_Berlin]

mean_data_Berlin <- mean(roi_Berlin)
Entropy_Berlin <- correa_estimator(roi_Berlin)
entropia_estimada_boot_Berlin <- bootstrap_correa_estimator(roi_Berlin, B)
True_entropy_Berlin <- (log(mean(roi_Berlin)) + (L - log(L) + lgamma(L) + (1 - L) * digamma(L)))
Test_difference_Berlin <- bootstrap_correa_estimator_log_mean(roi_Berlin, B) - ((L - log(L) + lgamma(L) + (1 - L) * digamma(L)))
...
centerRow_Flevoland <- floor(nRows_Flevoland / 2)
centerCol_Flevoland <- floor(nCols_Flevoland / 2)

roiSize_Flevoland <- 5

# ROI coordinates
roiStartRow_Flevoland <- centerRow_Flevoland - floor(roiSize_Flevoland / 2) + 1
roiEndRow_Flevoland <- roiStartRow_Flevoland + roiSize_Flevoland - 1

roiStartCol_Flevoland <- centerCol_Flevoland - floor(roiSize_Flevoland / 2) + 1
roiEndCol_Flevoland <- roiStartCol_Flevoland + roiSize_Flevoland - 1

# Extract the ROI
roi_Flevoland <- x_Flevoland[roiStartRow_Flevoland:roiEndRow_Flevoland, roiStartCol_Flevoland:roiEndCol_Flevoland]

mean_data_Flevoland <- mean(roi_Flevoland)
Entropy_Flevoland <- correa_estimator(roi_Flevoland)
entropia_estimada_boot_Flevoland <- bootstrap_correa_estimator(roi_Berlin, B)
True_entropy_Flevoland <- (log(mean(roi_Flevoland)) + (L - log(L) + lgamma(L) + (1 - L) * digamma(L)))
Test_difference_Flevoland <- bootstrap_correa_estimator_log_mean(roi_Flevoland, B) - ((L - log(L) + lgamma(L) + (1 - L) * digamma(L)))
...
# ROI coordinates in the upper part
roiUpperStartRow <- 1
...
# Extract the ROI in the upper part
roi_Ottawa <- x_Ottawa[roiUpperStartRow:roiUpperEndRow, roiUpperStartCol:roiUpperEndCol]

mean_data_Ottawa <- mean(roi_Ottawa)
Entropy_Ottawa <- correa_estimator(roi_Ottawa)
entropia_estimada_boot_Ottawa <- bootstrap_correa_estimator(roi_Ottawa, B)
True_entropy_Ottawa <- (log(mean(roi_Ottawa)) + (L - log(L) + lgamma(L) + (1 - L) * digamma(L)))
Test_difference_Ottawa <- bootstrap_correa_estimator_log_mean(roi_Ottawa, B) - ((L - log(L) + lgamma(L) + (1 - L) * digamma(L)))


# Save results
resultados_df <- data.frame(
  Ciudad = c("Amazonas", "Berlin", "Flevoland", "Ottawa"),
  Media = c(mean_data_brasil, mean_data_Berlin, mean_data_Flevoland, mean_data_Ottawa),
  Entropia_Estimada = c(Entropy_brasil, Entropy_Berlin, Entropy_Flevoland, Entropy_Ottawa),
  Entropia_Estimada_Boot = c(entropia_estimada_boot_brasil, entropia_estimada_boot_Berlin, entropia_estimada_boot_Flevoland, entropia_estimada_boot_Ottawa),
  Entropia_Verdadera = c(True_entropy_brasil, True_entropy_Berlin, True_entropy_Flevoland, True_entropy_Ottawa),
  Diferencia_Test_Entropia = c(Test_difference_brasil, Test_difference_Berlin, Test_difference_Flevoland, Test_difference_Ottawa)
)

colnames(resultados_df) <- c("Area", "Mean", "$\\widehat{H}_{\\text{C}}$", "$\\widetilde{H}_{\\text{C}}$", "$H_{\\Gamma_{\\text{SAR}}}$", "$S(\\bm Z)$")

save(resultados_df, file = "./Data/resultados_df.Rdata")


```

```{r Table_SAR_simulations, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
cat("\\setlength{\\tabcolsep}{4pt}\n")
 load("./Data/resultados_df.Rdata")

resultados_df[] <- lapply(resultados_df, function(x) {
  if (is.numeric(x)) {
    if (all(x %% 1 == 0)) {
      formatted_numbers <- sprintf("$%d$", x)
    } else {
...
print(
  kbl(
    resultados_df,
    caption = "Results of applying the statistical test to SAR images.",
    format = "latex",
    booktabs = TRUE,
    align = "lrrrrrrr",
    escape = FALSE,
