...
Synthetic aperture radar (SAR) has become a fundamental technology for environmental monitoring and disaster management because of its ability to provide daytime and nighttime imagery in all weather conditions&nbsp;[@Cassetti2022]. However, the utility of SAR data depends on a thorough understanding of the statistical properties governing its inherent speckle noise. This complex interaction, which defies simplistic assumptions of additivity and Gaussianity, requires robust statistical models that can accurately characterize SAR data for efficient processing and analysis.

Among these models, the $\mathcal{G}^0$ distribution stands out as a powerful framework for modeling SAR data. Notably, this distribution encompasses the well-known Gamma distribution as a special case, particularly under the assumption of fully developed speckle. The interplay between these two distributions is apparent, with the Gamma distribution representing a limiting case of the more general $\mathcal{G}^0$ model. 

On the other hand, entropy is a fundamental concept in information theory with broad applications in fields like classification, pattern recognition, statistical physics, stochastic dynamics, and statistics.
...
Estimating entropy faces practical challenges, particularly when the true probability density function (pdf) is unknown. Nonparametric methods are utilized in such cases, avoiding the assumption of a known pdf form. Instead, techniques like histograms or kernel density estimators are employed to estimate the pdf directly, followed by entropy calculation using numerical or Monte Carlo integration&nbsp;[@Lombardi2016]. An alternative nonparametric approach involves spacing methods, facilitating entropy estimation&nbsp;[@Zamanzade2012]. This non-parametric strategy offers flexibility to address a wide range of data distributions without imposing specific parametric constraints.

In this context, we present a two-stage estimation procedure designed to navigate the complexities of SAR data. The first stage of our methodology is to test the hypothesis that the data are completely speckled. If this hypothesis is rejected, we proceed to the second stage to estimate the parameters that characterize the $\mathcal{G}^0$ distribution.
...
The article is structured as follows: In Section&nbsp;\ref{sec:Background}, we provide an overview of statistical modeling for Intensity SAR data and the estimation of both parametric and non-parametric Shannon entropy.
...
General asymptotic results for functions of spacings are detailed in&nbsp;[@Khashimov1990], while VanEs&nbsp;[@Bert1992] has developed a correction for the case of Shannon entropy.
...
Fig.&nbsp;\ref{fig:3d_GIO} illustrates the entropy of $\mathcal{G}_I^0$ distribution as a function of three key parameters: $\mu$, $\alpha$, and $L$.
...
```
As we explore the 3D plot, we can observe how changes in $\mu$, $\alpha$, and $L$ collectively influence the Entropy of the GI0 distribution. We can identify regions where Entropy is high or low, providing insights into the predictability and structure of the distribution in various regions of the parameter space.
