...
\renewcommand{\tablename}{\MakeUppercase{\oldtablename}}
...
In Fig.&nbsp;\ref{fig:Plot_GI0_to_gamma1}, we depict the behavior of the entropy of \(G_I^0\) when \(\alpha \in \left\{-\infty, -20, -8, -3\right\}\), observing its convergence towards the entropy of \(\Gamma_{\text{SAR}}\) as \(\alpha\) takes large negative values. 


```{r Plot_GI0_to_gamma1, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center", fig.pos="hbt", fig.cap="$H_{ G_I^0}$ converges to the $H_{\\Gamma_{\\text{SAR}}}$, $L=8$."}
...
For brevity, we present the results with \(\mu=1\), \(\alpha=-10\), and \(L=5\). 
...
```{r Plot_bias_mse_gi0, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",  fig.cap="Bias and MSE, $L=5$ and  $\\alpha=-10$.", fig.width=6, fig.height=3.0}
...
print(
  kbl(
    reshaped_results,
    caption = "Bias and MSE of bootstrap estimators.",
    format = "latex",
    booktabs = TRUE,
    align = "lrrrrrrrr",
    escape = FALSE, 
...
## The Proposed Test
Fig.&nbsp;\ref{fig:Plot_density} depicts empirical test statistic densities for various sample sizes, and Table&nbsp;\ref{tab:table_stat_combined} summarizes key statistics, including mean, SD, variance (Var), coefficient of variation (CV), skewness (SK), excess kurtosis (EK), and Anderson–Darling $p$-values for normality. 
<!-- ACF Only show L=2 and L=8 -->
Results with $p$-values larger than $0.05$  indicate no evidence against normality. 
Small variance and near-zero skewness and excess kurtosis suggest limited spread, symmetry, and light tails. Increasing sample size reduces variability relative to the mean. 
...
```{r Plot_density, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",  fig.cap="Empirical density  under null hypothesis.", fig.width=6, fig.height=3.0}
...
colnames(combined_summary_stats) <- c("$L$", "$n$", "Mean", "SD", "Var", "CV", "SK", "EK", "$p$-value")
...
# 
save(TestStatistics1, TestStatistics2, file = "./Data/sim_empirical_test1.Rdata")
...
# 
load("./Data/sim_empirical_test1.Rdata")
...
We used $7 \times 7$ local sliding windows for the test. 
Fig.&nbsp;\ref{fig:Real_SAR_Images}(b) shows the heatmap of the $p$-values.
The color table maps all above $0.05$ into dark blue (no evidence to reject the fully-developed speckly hypothesis), and those below  $0.05$ into a gradient between blue and orange (evidence to reject the hypothesis).


```{r Real_SAR_Images, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center",fig.pos = 'H', fig.cap='Results of applying the test statistical to actual data.',  fig.subcap=c('SAR image.', '$p$-values.    ' ), out.width = c("40mm", "59mm"),  fig.ncol=2, fig.width = 20, fig.height = 10}
knitr::include_graphics("../../../Figures/PNG/Intensity.png")#out.width='.55\\linewidth'
knitr::include_graphics("../../../Figures/PDF/pvalue_f.pdf")
```

These results show that the map of $p$-values correctly identify regions with different levels of heterogeneity.
...
We proposed a testing procedure for distinguishing between fully-developed speckle and heterogeneous clutter. This tool is equipped with a test statistic based on a bootstrap-improved estimator of the Shannon entropy. 
...
Using empirical test size and power as figures of merit, our proposal was assessed for various sample sizes, numbers of looks, and nominal levels. 

Results showed that the test presented good performance for sample sizes larger than $50$, scenarios that frequently occur in SAR image processing practice.

Applying the test to actual SAR data yielded promising results. Different regions in the image were effectively distinguished and the potential of the test to distinguish between homogeneous and heterogeneous zones was demonstrated.

# Acknowledgement {-}

This study was financed in part by the Coordenação de Aperfeiçoamento de Pessoal de Nível Superior  (CAPES), the Fundação de Amparo à Ciência e Tecnologia de Pernambuco (FACEPE), and National Council for Scientific and Technological Development, Brazil.
