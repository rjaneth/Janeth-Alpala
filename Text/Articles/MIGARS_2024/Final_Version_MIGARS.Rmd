...
For this purpose, we propose the following test statistic:
\begin{equation}
\label{Eq:test_e}
S(\bm{Z};L_0)= \widetilde{H}_{\text{C}}(\bm{Z})-\left[H_{\Gamma_{\text{SAR}}}(L_0)+\ln \widebar{\bm{Z}}\right].
\end{equation}
This test statistic $S(\bm{Z};L_0)$ aims to assess the behavior of the data under the null hypothesis, through the empirical distribution. 
...
Small variance and near-zero skewness and kurtosis suggest limited spread, symmetry, and light tails. The coefficient of variation shows that as the sample size increases, variability with respect to the mean decreases.
...
$$
Nominal levels of $1\%$, $5\%$, and $10\%$ were considered. In reference to the size, $1000$ simulations have been used for various sample sizes with  satisfactory results. In all cases, the nominal level has been achieved. The power, in general, improves with the increase in the sample size and the number of looks. The results are presented in Table&nbsp;\ref{tab:table_size_power}.




```{r Simulated_error_type_I, echo=FALSE, message=FALSE, cache = TRUE, autodep = TRUE}
set.seed(1234567890, kind = "Mersenne-Twister")
...
```{r Simulated_power, echo=FALSE, message=FALSE, cache = TRUE, autodep = TRUE}
...
Fig.&nbsp;\ref{fig:Plot_empirical_test} displays the empirical distributions using the test statistical&nbsp;\eqref{Eq:test_e}. Under the null hypothesis, the density centers around zero. 
...
```{r Plot_empirical_test, echo=FALSE, message=FALSE, warning=FALSE, out.width="90%", fig.show="hold", fig.align="center", fig.pos="hbt", fig.cap="Empirical distributions under null hypothesis (centered around zero) and alternative hypothesis (e.g., $\\alpha=-8$, showing a shift from zero), with $\\mu=1$ and $L=5$."}
...
for (alpha_val in alpha1_values) {
  plot <- ggplot(TestStatistics1, aes(x = Test_Statistics)) +
    ...
    annotate("text", x=difference.betweeen.GammaSAR.GI0.n.Look(-8, 5), y=6, parse=TRUE, label="alpha==-8", hjust=-.1, vjust=1.0, col="darkgreen") +
...
We applied the test statistical to a real SAR image of Flevoland province, Netherlands, acquired by the Sentinel-1A system operating in the C band, with  HH polarization, and intensity format.
This image has $300 \times 300$ pixels and covers two different areas, urban areas and agricultural fields. 
The metadata associated with these original SAR images includes a number of looks $L = 5$.

Fig.&nbsp;\ref{fig:Real_SAR_Images}(a) shows the SAR image, from which we selected $7 \times 7$ local sliding windows covering the entire image. 
The test statistical is applied to each window. 
In Fig.&nbsp;\ref{fig:Real_SAR_Images}(b), the test results and corresponding $p$-values, computed over each $7\times 7$ sample, are shown as pixels. 
This assessment aims to determine the test's effectiveness in identifying data origin. 
The new image reveals two distinct regions: lighter backgrounds indicate homogeneity, while darker backgrounds represent heterogeneous areas.





```{r Real_SAR_Images, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", fig.show="hold", fig.align="center", fig.cap='Results of applying the test statistical to actual data.', fig.subcap=c('SAR image.', '$p$-values calculated.' ), out.width='.48\\linewidth',  fig.ncol=2}
...
Conclusively, in this work, we propose a testing procedure for distinguishing between fully-developed speckle and heterogeneous clutter. 
We focus on the estimation of Shannon entropy using non-parametric estimators, enhancing precision through bootstrap techniques. 
...
Furthermore, our assessment of the proposed test involved the calculation of empirical size and power, considering various sample sizes, numbers of looks, and nominal levels. 
The results demonstrate its capabilities under both null and alternative hypotheses. Notably, the test performs acceptably with sample sizes larger than 50. 
