...
\end{equation}
This test statistic $S(\bm{Z};L_0)$ aims to assess the behavior of the data under the null hypothesis, through the empirical distribution of the test statistic values for a sample size of $n$. The density should center around zero if the test statistic is capable of detecting that the data are fully-developed speckle, i.e., $S(\bm{Z};L_0)\approx 0$. Otherwise, under the alternative hypothesis, the empirical distribution of the test statistic values is not centered around zero. This suggests significant differences and implies the presence of heterogeneous clutter.
...
Fig. \ref{fig:Plot_density} depicts empirical test statistic densities for various sample sizes, and Table \ref{tab:table_stat_combined} summarizes key statistics, including mean, SD, variance (Var), skewness, kurtosis, and Anderson–Darling $p$-values for normality. Results with $p$-values $> 0.05$  indicate no evidence against normality. 
...
for (L in L_values) {
  ...
  for (s in sample.size) {
   ...
   # lillie_p_value <- lillie.test(TestStatistics1[[as.character(s)]]$Test_Statistics)$p.value
    ...
    # Almacenar resultados de normalidad
...
# for (L in L_values) {
#   summary_stats <- all_summary_stats[[as.character(L)]]
#   
#   print(summary_stats)
# }
...
# Tamaño de muestra específico que deseas graficar
selected_sample_size <- 100

# Crear una lista para almacenar los ggplots
ggplot_list <- list()
...
for (L in c(2, 5, 8, 18)) {
  ...
  # Verifica si el tamaño de muestra seleccionado está disponible
