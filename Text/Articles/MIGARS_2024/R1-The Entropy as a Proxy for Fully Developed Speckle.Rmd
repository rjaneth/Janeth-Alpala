...
B <- 1
alpha1 <- -8
sample.size <- c(50, 100, 150, 200, 250,300)
L_values <- c(2, 5, 8, 18)
...
# LaTeX notation for negative numbers and integers
  ...
  for (s in sample.size) {
    ...
    for (r in 1:R) {
      ...
      TestStat2[r] <- bootstrap_correa_estimator_log_mean(z1, B) - (L - log(L) + lgamma(L) + (1 - L) * digamma(L))
        #(L - log(L) + lgamma(L) + (1 - L) * digamma(L) - L - lgamma(L - alpha1) + (L - alpha1) * (digamma(L - alpha1))- (1 - alpha1) * digamma(-alpha1) + log(-1 - alpha1) + lgamma(-alpha1))
