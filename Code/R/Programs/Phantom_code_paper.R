...
# codigo que genera el phantom del paper mdpi
...
z.up.le.b <- rGI0(n=sum(1-p.up.le), 
                  p_alpha=-6, p_mu=1, p_Looks=5)
mean(z.up.le.b)

z.up.le.w <- rGI0(n=sum(p.up.le), 
                  p_alpha=-1.5, p_mu=20, p_Looks=5)
mean(z.up.le.w)
z.up.le <- p.up.le
...
z.up.ri.b <- rGI0(n=sum(1-p.up.ri), 
                  p_alpha=-8, p_mu=2, p_Looks=5)
mean(z.up.ri.b)

z.up.ri.w <- rGI0(n=sum(p.up.ri), 
                  p_alpha=-2, p_mu=30, p_Looks=5)
mean(z.up.ri.w)
z.up.ri <- p.up.ri
...
z.bo.ri.b <- rGI0(n=sum(1-p.bo.ri), 
                  p_alpha=-20, p_mu=4, p_Looks=5)
mean(z.bo.ri.b)

z.bo.ri.w <- rGI0(n=sum(p.bo.ri), 
                  p_alpha=-3, p_mu=100, p_Looks=5)
mean(z.bo.ri.w)
z.bo.ri <- p.bo.ri
...
z.bo.le.b <- rGI0(n=sum(1-p.bo.le), 
                  p_alpha=-10, p_mu=3, p_Looks=5)
mean(z.bo.le.b)
z.bo.le.w <- rGI0(n=sum(p.bo.le), 
                  p_alpha=-2.5, p_mu=50, p_Looks=5)
mean(z.bo.le.w)
z.bo.le <- p.bo.le
...
#save(Z, file="./Data/Phantom_4_z.Rdata")