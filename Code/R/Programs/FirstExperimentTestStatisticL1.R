...
ggplot(TestStatistics, aes(x = `Test Statistics`, col = factor(`Sample Size`), linetype = factor(`Sample Size`))) +
  ...
  annotate("text", x=difference.betweeen.GammaSAR.GI0.OneLook(-1.01), y=15, parse=TRUE, label="alpha==-1.01", hjust=-0.1, col="red") +
  geom_vline(xintercept = difference.betweeen.GammaSAR.GI0.OneLook(-1.1), col="red") +
  annotate("text", x=difference.betweeen.GammaSAR.GI0.OneLook(-1.1), y=15, parse=TRUE, label="alpha==-1.1", hjust=-0.1, col="red") +
  geom_vline(xintercept = difference.betweeen.GammaSAR.GI0.OneLook(-1.5), col="red") +
  annotate("text", x=difference.betweeen.GammaSAR.GI0.OneLook(-1.5), y=15, parse=TRUE, label="alpha==-1.5", hjust=1.1, vjust=2, col="red") +
